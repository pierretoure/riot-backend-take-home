...
/**
 * Contract test for the `Cipher` port (cahier des charges §3.2/§10): every
 * adapter must satisfy the same behavioural contract regardless of its
 * concrete algorithm. Run via `describe.each` against both `Base64Cipher`
 * (the one actually wired in `crypto.module.ts`) and `RotCipher` (kept
 * unused, purely to prove the abstraction holds).
 */
...
describe.each(adapters)('$name (Cipher contract)', ({ create }) => {
  ...
  it('looksEncrypted(encrypt(x)) is true for JSON-text input', () => {
    ...
    // (§4.1), which is exactly what `Base64Cipher.looksEncrypted`'s fourth
    // criterion expects back after decoding. Samples here are therefore
    // valid JSON text, matching real usage.
    //
    // The empty string is excluded for a different reason:
    // `Base64Cipher.looksEncrypted`'s first criterion (§4.2) requires at
    // least one Base64 character, so `encrypt('')` (which is `''`) cannot
    // satisfy the contract — a known, documented edge case rather than a bug.
